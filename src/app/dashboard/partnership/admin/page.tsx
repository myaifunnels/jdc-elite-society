...
type PartnershipAdminTab = "actions" | "payouts" | "campaigns";

function parseAdminTab(value?: string): PartnershipAdminTab {
  return value === "payouts" || value === "campaigns" ? value : "actions";
}
    ...
    <>
      <div className="macos-toolbar" style={{ padding: "0 0 0.9rem" }}>
        <div className="macos-segment" style={{ gridTemplateColumns: "1fr 1fr 1fr", width: "min(26rem, 100%)" }}>
          <Link href="/dashboard/partnership/admin" className={cn(tab === "actions" && "is-active")}>
            Actions
          </Link>
          <Link href="/dashboard/partnership/admin?tab=payouts" className={cn(tab === "payouts" && "is-active")}>
            Payouts
          </Link>
          <Link href="/dashboard/partnership/admin?tab=campaigns" className={cn(tab === "campaigns" && "is-active")}>
            Campaigns
          </Link>
        </div>
      </div>
                  ...
                  return (
                    <div key={sale.id} className="dashboard-contact-row !cursor-default">
                      <span>
                        ...
                        <em>
                          {formatPhp(sale.commissionAmount)} · due {formatManilaDate(sale.scheduledPayDate)} ·{" "}
                          {sale.status}
                        </em>
                      </span>
