...
import { notifyPartnershipQualified } from "@/lib/notify";

export const dynamic = "force-dynamic";

/**
 * GHL workflow target for when a contact moves to the "Qualified" stage in the JDC Partnership
 * Program pipeline: add a Webhook action to that trigger pointing here, and the applicant gets
 * the "you're approved" SMS. The matching email is sent separately from the same GHL workflow
 * using email-templates/partnership-qualified.html.
 */

export async function GET() {
  return NextResponse.json({ ok: true, service: "partnership-ghl-webhook-qualified" });
}
