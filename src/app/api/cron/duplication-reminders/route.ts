...
import { sendDueDuplicationReminders } from "@/lib/duplication-reminders";
 ...
 * way — a stage is only ever sent once per enrollee. */
export async function GET(request: NextRequest) {
  ...
  try {
    const result = await sendDueDuplicationReminders();
    return NextResponse.json({ ok: true, ...result });
  } catch (error) {
    console.error("Duplication reminder cron failed", error);
